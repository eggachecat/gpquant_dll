...
namespace GPQuant
{
	double* BackTesting::cheating(int* indices, double* y_pred, int n_data, double* price_table, int n_dim, int x_len) {
		...
		for (int i = 0; i < n_data; i++)
		{
			...
			if (op[idx] != 0) {
				...
				if (profit < 0 && x_len < 0) {
					cout << profit << " " << i << " " << op[idx] << " " << invest << " " << pre_invest << endl;
				}
			...
			profit_arr[i] = profit;
			if (x_len < 0) {
				cout << "idx: " << idx << ", op: " << op[idx] << ", y_pred: " << y_pred[idx] << ", price: " << price_table[idx] << ", invest: " << invest << ", fund: " << profit_arr[i] << endl;;
			}
		}
